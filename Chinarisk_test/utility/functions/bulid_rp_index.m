function [simulated_nav,weight,w] = bulid_rp_index(p,tickers,w_,rtn_v_table, rtn_p_table)
    ...
    [w, trans_cost] = deal(zeros(length(p.reblance_dates),size(rtn_v_table,2)-1));
    
    % update w 
    for i  = 1 : length(p.reblance_dates)  
        %disp(i)
         if  p.reblance_dates(i) >= addtodate(rtn_v_table.DATEN(1), p.corr_dates, 'day')
                        
             id2 = find(p.reblance_dates(i)==rtn_v_table.DATEN,1,'first');
             id1 = find(addtodate(p.reblance_dates(i), -p.corr_dates, 'day')<=rtn_v_table.DATEN,1,'first');
    ...
    weight_table = array2table([p.reblance_dates,w],         'VariableNames',rtn_v_table.Properties.VariableNames);
    cost_table   = array2table([p.reblance_dates,trans_cost],'VariableNames',rtn_v_table.Properties.VariableNames);
    
   w   = array2table([p.reblance_dates,w],'VariableNames',rtn_v_table.Properties.VariableNames);
